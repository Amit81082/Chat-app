...
import DeleteChatBtn from "./DeleteBtn";
...
const MessagePage = () => {
  ...
  console.log("current user ki details", user);
  ...
  useEffect(() => {
    const handleClickOutside = (e) => {
      if (menuRef.current && !menuRef.current.contains(e.target)) {
        setOpenImageVideoUpload(false); // 👈 CLOSE
      }
    };
  ...
  useEffect(() => {
    ...
    const sendMessagePage = () => {
      if (lastUserRef.current !== params?.userId) {
        lastUserRef.current = params?.userId;
    ...
    // 👉 FIX 1: handle connection properly
    if (socketConnection.connected) {
      sendMessagePage();
    } else {
      socketConnection.once("connect", sendMessagePage);
    }

    const handleUser = (data) => {
      console.log("msg user data", data);
      setDataUser(data);
    };
  ...
  const handleSendMessage = async (e) => {
    ...
    if (message.text || message.imageUrl || message.videoUrl) {
      if (socketConnection) {
        await socketConnection.emit("new message", {
          sender: user?._id,
  ...
  const handleDeleteChat = async () => {
    if (!socketConnection || !user?._id) return;
    await socketConnection.emit("delete-chat", {
      userId: user?._id,
  ...
  return (
    ...
    >
      ...
      {/**send message */}
      <section className="h-16 bg-white flex items-center px-4 sticky bottom-0 ">
        <div className="relative ">
          <button
